...
// Декларация функций для потоков
...
int main(int argc, char const *argv[]) {
    try {
        ...
        std::future<const path> thr1(std::async(get_name, filePath)); // поток для определения имени файла
        std::future<double> thr2(std::async(get_size, filePath)); // для размера
        std::future<void> thr3(std::async(std::launch::async, get_data, filePath)); // для времени изменения

        thr1.wait(); // ждет выполнения первого потока
        thr2.wait(); // ждет выполнения второго потока
